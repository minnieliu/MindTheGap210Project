...
public class ArrivalsListFragment extends ListFragment {
    ...
    public ArrayList<Arrival> getArrivalsForSelectedStationOnLineInDirection(String lineId, String travelDirn) {
        ...
        Station selectedStn = stnManager.getSelected();
        ArrayList<ArrivalBoard> boards = new ArrayList<>();
        ArrayList<Arrival> arrivals = new ArrayList<>();

        if (selectedStn != null) {
            Iterator<ArrivalBoard> iterableArrivalBaords = selectedStn.iterator();
            if (iterableArrivalBaords != null) {
                while (iterableArrivalBaords.hasNext()) {
                    boards.add(iterableArrivalBaords.next());
                }
            }
        }
        for (ArrivalBoard ab :boards){
            if (ab.getLine().getId().equals(lineId) &&
                    ab.getTravelDirn().equals(travelDirn)){
                Iterator<Arrival> iterableArrivals = ab.iterator();
                if (iterableArrivals != null){
                    while (iterableArrivals.hasNext()){
                        arrivals.add(iterableArrivals.next());
                    }
                }
            }
        } return arrivals;
